...
const CeldaDato = ({ columna, valor }) => {
    if (columna.tipo === 'image') {
        return valor ? (
            <img src={valor} alt="Foto" className="tabla-img-preview" onError={(e) => e.target.style.display = 'none'} />
        ) : (
            <span className="tabla-no-img">—</span>
        );
    }
    if (columna.tipo === 'badge') {
        const clase = obtenerClaseBadge(columna.key, valor);
        return <span className={`tabla-badge ${clase}`}>{valor || '—'}</span>;
    }
...
const FilaTabla = ({ registro, columnas, onVer, onEditar, onEliminar }) => (
    <tr key={registro.id}>
        ...
        <td className="tabla-col-acciones">
            <button className="btn-accion btn-ver" onClick={() => onVer(registro)} title="Ver detalle">
                <i className="bi bi-eye-fill"></i> Ver
            </button>
            <button className="btn-accion btn-editar" onClick={() => onEditar(registro)} title="Editar registro">
                <i className="bi bi-pencil-fill"></i> Editar
            </button>
            <button className="btn-accion btn-eliminar" onClick={() => onEliminar(registro)} title="Eliminar registro">
...
const TablaGeneral = ({ titulo, icono, columnas = [], registros = [], loading = false, error = null, onVer, onEditar, onEliminar }) => {
    return (
        <div className="tabla-glass-container" translate="no">
